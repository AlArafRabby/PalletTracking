...
{
    public partial class PalletTransferOut : UserControl
    {
        ...
        private void PalletTransferOut_Load(object sender, EventArgs e)
        {
            ...
            DT = _PalletTransferin.PlateTransferIn(10, xml, 0, 0, "", ref Msg);
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    DateTime.Parse(txtTransectionDate.Text) <= DateTime.Now && DateDiffrence())
                    {
                        ...
                        DT = _PalletTransferin.PlateTransferIn(6, xml, whId, plateId, barcode, ref Msg);           
                        BindGread(DT);
                        if (Msg.Length > 1)
                        {
                            lblMessege.Text = Msg;
        ...
        public void BindGrid()
        {
            try
            {
                ...
                string xml = "<voucher><voucherentry shift=" + '"' + ddlShift.SelectedItem.ToString() + '"' + " intActionBy=" +
                             '"' + Enroll + '"' + " dteTransection=" + '"' + dteTransection + '"' + "/></voucher>".ToString();
                DT = _PalletTransferin.PlateTransferEntry(8, xml, whId, 0, "0", ref Msg);
                BindGread(DT);
                if (Msg.Length > 1)
                {
                    lblMessege.Text = Msg;
        ...
        private void ShowTransferOut_Click(object sender, EventArgs e)
        {
            BindGrid();
        }
    }
