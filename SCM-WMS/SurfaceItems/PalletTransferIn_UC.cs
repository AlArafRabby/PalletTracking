...
{
    public partial class PalletTransferIn_UC : UserControl
    {
        ...
        private void PalletTransferIn_UC_Load(object sender, EventArgs e)
        {


            string xml = "<voucher><voucherentry shift=" + '"' + 0 + '"' + " intActionBy=" +
                                    '"' + Enroll + '"' + "/></voucher>".ToString();


            DT = _PalletTransferin.PlateTransferIn(4, xml, 0, 0, "", ref Msg);
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    DateTime.Parse(txtTransectionDate.Text) <= DateTime.Now && DateDiffrence())
                    {
                        ...
                        DT = _PalletTransferin.PlateTransferIn(1, xml, whId, plateId, barcode, ref Msg);
        ...
        public void BindGrid()
        {
            try
            {
                ...
                DateTime dteTransection = DateTime.Parse(txtTransectionDate.Text);
                string xml = "<voucher><voucherentry shift=" + '"' + ddlShift.SelectedItem.ToString() + '"' + " intActionBy=" +
                                       '"' + Enroll + '"' + " dteTransection=" + '"' + dteTransection.ToString() + '"' + "/></voucher>".ToString();
                DT = _PalletTransferin.PlateTransferEntry(7, xml, whId, 0, "0", ref Msg);
                lblMessege.Text = Msg;
                BindGread(DT);
                txtBarcode.Text = "";
                txtBarcode.Focus();
